    ...
    public class func locationWhenInUseUsage(isRequest: Bool = true, completionHandler handler: @escaping (CLAuthorizationStatus) -> Void) {
        if CLLocationManager.locationServicesEnabled() {
            ...
            if #available(iOS 14.0, *) {
                authorizationStatus = PTLocationManager.default.locationManager.authorizationStatus
            } else {
            ...
            case .notDetermined:
                if isRequest {
                    PTLocationManager.default.handler = handler
                    PTLocationManager.default.locationManager.requestWhenInUseAuthorization()
                } else {
                    handler(authorizationStatus)
                }
            default:
    ...
    public class func locationAlwaysUsage(isRequest: Bool = true, completionHandler handler: @escaping (CLAuthorizationStatus) -> Void) {
        if CLLocationManager.locationServicesEnabled() {
            ...
            case .notDetermined:
                if isRequest {
                    PTLocationManager.default.handler = handler
                    PTLocationManager.default.locationManager.requestAlwaysAuthorization()
                } else {
                    handler(authorizationStatus)
                }
            default:
...
//MARK: - 定位管理
public class PTLocationManager: NSObject {

    public static let `default` = PTLocationManager()
...
extension PTLocationManager: CLLocationManagerDelegate {

    public func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        guard let handler = PTLocationManager.default.handler else { return }
        handler(status)
    }
